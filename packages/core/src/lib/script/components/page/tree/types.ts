 ...
 *
 * A node now carries `publicationId`. Publishing pins the revision current at that moment, and the
 ...
 * name and its attribute schema. There is nothing for the CMS to pin, so `uid` and `publicationId` are
...
interface ReadablePageNode {
   ...
   *
   * Present exactly when `publicationId` is: together they name the artifact to fetch. Absent for a
