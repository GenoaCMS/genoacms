# GenoaCMS consumer demo — React

One CMS-authored page, fetched, verified and rendered by a React application holding nothing but a
32-byte public key.
...
| | |
| :--- | :--- |
| **`src/genoa/`** | **Boilerplate.** The wrapper that turns the SDK's resolved tree into React components. You copy this once and rarely touch it. Here it is `src/genoa/GenoaComponent.tsx`. |
| **`src/components/`** | **Yours.** Ordinary React components that know nothing about GenoaCMS, plus the map saying which published component each one implements and in what order its values arrive. |
...
pnpm --filter @genoacms/demo-react dev
```
