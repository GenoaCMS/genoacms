# GenoaCMS consumer demo — Vue

One CMS-authored page, fetched, verified and rendered by a Vue application holding nothing but a
32-byte public key.
...
| | |
| :--- | :--- |
| **`src/genoa/`** | **Boilerplate.** The wrapper that turns the SDK's resolved tree into Vue components. You copy this once and rarely touch it. Here it is `src/genoa/GenoaComponent.ts`. |
| **`src/components/`** | **Yours.** Ordinary Vue components that know nothing about GenoaCMS, plus the map saying which published component each one implements and in what order its values arrive. |
...
pnpm --filter @genoacms/demo-vue dev
```
