 ...
 * ## What spends them
 *
 * Fuel at every loop header and every declared function, depth around every declared function, and
 * allocation at every sized construction and every concatenation inside a loop. `instrument.ts`
 * explains why none of those calls costs a line.
 */
 ...
 *     3. charge allocation over expressions, which the first two only ever wrapped around
 *     4. instantiate       one line, inside the entry function, paid for by the prologue
 *     5. append helper     after everything, hoisted, moving nothing
 *
 ...
 * `return`. Allocation reads expressions rather than bodies, so it could share a parse with neither
 * without its offsets being computed against a source the others had already changed.
 *
...
const injectGuards = (
  ...
  const bounded = parse(boundDepth(fueled.getFullText(), entryFunctionOf(fueled), guards))
  const file = parse(chargeAllocation(bounded.getFullText(), entryFunctionOf(bounded), guards))
